...
parser.add_argument('--tmp_data_dir', type=str, default='../data1', help='temp data dir')
...
parser.add_argument('--sep_loss', type=str, default='l2', help='which loss to use?l1 or l2')
parser.add_argument('--aux_loss_weight', type=float, default=10, help='weight decay')
    ...
    #*******************************************************************************************8
        ...
        for epoch in range(epochs[sp]):
            ...
            if epoch < eps_no_arch:
                model.module.p = float(drop_rate[sp]) * (epochs[sp]-epoch - 1) / epochs[sp]    #！！！
                model.module.update_p()
                train_acc, train_obj = train(train_queue, valid_queue, model, network_params, criterion_train, optimizer, optimizer_a, lr, train_arch=False)
            else:
                model.module.p = float(drop_rate[sp]) * np.exp(-(epoch - eps_no_arch) * scale_factor) 
                model.module.update_p()                
                train_acc, train_obj = train(train_queue, valid_queue, model, network_params, criterion_train, optimizer, optimizer_a, lr, train_arch=True)
            ...
            shengcheng(arch_normal,arch_reduce,switches_normal,switches_reduce,betas_nor,betas_redu)
            #/***********************************************************/
...
def train(train_queue, valid_queue, model, network_params, criterion, optimizer, optimizer_a, lr, train_arch=True):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        if train_arch:
            ...
            aux_input = torch.cat([F.sigmoid(model.module.arch_parameters()[0]), F.sigmoid(model.module.arch_parameters()[1])], dim=0)   #***
            loss_a = criterion(logits, target_search,aux_input)
            loss_a.backward()
        ...
        logits = model(input)
        aux_input = torch.cat([F.sigmoid(model.module.arch_parameters()[0]), F.sigmoid(model.module.arch_parameters()[1])], dim=0)   #***
        loss = criterion(logits, target,aux_input)
